...
const CustomerPortal = (props) => {
  ...
  const {selectedSubType, customerData,details,customerPortalForm,setSelectedSubType,selectedSubTypeId,cursorPortalLU,clientEnquiryData,requestModeLU} = props;
  ...
  useEffect(()=>{
    if(props?.EmailResponse?.IsEmailmanagent){
      CustomerPortalData[selectedSubType]?.BOE_Details?.forEach(element => {
    ...
    // CustomerPortalData[selectedSubType]?.BOE_Details?.forEach(element => {
      if (!CustomerPortalData[selectedSubType]) {
        CustomerPortalData[selectedSubType] = {}; // Initialize if undefined
      }
      
      if (!Array.isArray(CustomerPortalData[selectedSubType]?.BOE_Details)) {
        CustomerPortalData[selectedSubType].BOE_Details = [];
      }
      
      // Remove existing instances of "Additional Note For Customer" before adding a new one
      CustomerPortalData[selectedSubType].BOE_Details = CustomerPortalData[selectedSubType].BOE_Details.filter(
        comment => comment.name !== "AdditionalNoteForCustomer"
      );
      
      // Add "Additional Note For Customer" once
      CustomerPortalData[selectedSubType].BOE_Details.push(
  ...
  const handleDropdownChange=(e,item)=>{
    if(item?.name==="Selectportalissue"){
