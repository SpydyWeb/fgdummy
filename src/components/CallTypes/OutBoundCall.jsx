...
const OutBoundCall = (props) => {
  ...
  const {selectedSubType, customerData,details,callRelatedActionLU,clientEnquiryData,requestModeLU } = props;
  ...
  useEffect(()=>{
     if(props?.EmailResponse?.IsEmailmanagent){
      OutBoundCallData[selectedSubType]?.BOE_Details?.forEach(element => {
        if (element?.name === "requestchannel") {
            form.setFieldsValue({ requestchannel: 4 });
            element.disabled = true;
        }
    });
      if (!OutBoundCallData[selectedSubType]) {
        OutBoundCallData[selectedSubType] = {}; // Initialize if undefined
      }
      if (!Array.isArray(OutBoundCallData[selectedSubType]?.BOE_Details)) {
        OutBoundCallData[selectedSubType].BOE_Details = [];
        }
      
        // Remove existing instances of "Additional Note For Customer" before adding a new one
        OutBoundCallData[selectedSubType].BOE_Details = OutBoundCallData[selectedSubType].BOE_Details.filter(
            comment => comment.name !== "AdditionalNoteForCustomer"
        );
      
        // Add "Additional Note For Customer" once
        OutBoundCallData[selectedSubType].BOE_Details.push(
    ...
    if(selectedSubType==="welcomecallconcernraised" || selectedSubType==="welcomecallcompleted"  ||
      selectedSubType==="incorrectdetailsinpolicybond")//Addec by sayali on 23-07-2025 of set welcomecall default value
    {
      OutBoundCallData[selectedSubType]?.BOE_Details?.forEach(element => {
        if (element?.name === "requestchannel") {
            form.setFieldsValue({ requestchannel: 10});
...
//     setRequirementLoader(true);
 if(raiseRequirementOpen){
  ...
  //Remove policyDetails? in all js file by sayali on 24/07/2025
  const handleSubmit = (values) => {
    ...
    const obj = {
      ...
      Category: selectedSubType === "privacydonotcall" || raiseRequirementOpen ? 2 : 1,
      ApplicationNo:
      details?.policyDetailsObj?.identifiers?.applicationNo ||customerData?.applicationNo,//remove policyDetails?
      DOB: convertDate(customerData?.dob),
      PolicyNo: details?.policyDetailsObj?.identifiers?.policyNo || customerData?.policyNo, // Required //remove policyDetails?
      CustomerId: 456,
      CustRole: values?.custRole,
      policyStatus:
      details?.policyDetailsObj?.planAndStatus?.policyStatus || customerData?.policyStatus,//remove policyDetails?
      proposerName: details?.policyDetailsObj?.identifiers?.po_Name || customerData?.po_Name,//remove policyDetails?
      plan: details?.policyDetailsObj?.planAndStatus?.planName || customerData?.planName,//remove policyDetails?
      CreatedOn: new Date(),
    ...
    <>
      <Spin spinning={isLoading}>
        ...
        >
          ...
          ></DetailsForm>
          {(selectedSubType !== "privacydonotcall"||!isDNDFlag || loginInfo?.userProfileInfo?.profileObj?.role=== 1)&&<>
          <div className="contact-details-btn">
                ...
                {selectedSubType !== "wrongcustomercontacted" && <>
