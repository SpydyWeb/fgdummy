...
const GeneralInformation = (props) => {
  ...
  const { selectedSubType, customerData, details,clientEnquiryData,requestModeLU } = props;
  ...
  useEffect(()=>{
    form?.setFieldsValue({
      CareID: import.meta.env.VITE_APP_CAREMAIL,
      TollFreeNumber: import.meta.env.VITE_APP_GENERALINFO_TOLLFREENO,
      WhatsAppNumber: import.meta.env.VITE_APP_GENERALINFO_NO,
  ...
  useEffect(()=>{
    if(props?.EmailResponse?.IsEmailmanagent){
      GeneralInformationData[selectedSubType]?.BOE_Details?.forEach(element => {
        if (element?.name === "requestchannel") {
            form.setFieldsValue({ requestchannel: 4 });
            element.disabled = true;
        }
    });
      if(selectedSubType==='others'||selectedSubType==='touchpointinformation'){

      if (!GeneralInformationData[selectedSubType]) {
        GeneralInformationData[selectedSubType] = {}; // Initialize if undefined
      }
      if (!Array.isArray(GeneralInformationData[selectedSubType]?.BOE_Details)) {
        GeneralInformationData[selectedSubType].BOE_Details = [];
    }
  
    // Remove existing instances of "Additional Note For Customer" before adding a new one
    GeneralInformationData[selectedSubType].BOE_Details = GeneralInformationData[selectedSubType].BOE_Details.filter(
        comment => comment.name !== "AdditionalNoteForCustomer"
    );
  
    // Add "Additional Note For Customer" once
    GeneralInformationData[selectedSubType].BOE_Details.push(
  ...
  const getTransactionData = (values) => {
    if (selectedSubType === "touchpointinformation") {
      return [
        ...
        { Status: "Create", TagName: "MobileNumber", TagValue: values.mobileNo },
        { Status: "Create", TagName: "AdditionalNoteForCustomer", TagValue: values?.AdditionalNoteForCustomer?.replace(/\\n|\n/g, " ") || "" },
        // { Status: "Create", TagName: "MobilAeppIOS", TagValue: isMobileAppIOSChecked ? linksData[3]?.value[0]?.mstDesc : null },
