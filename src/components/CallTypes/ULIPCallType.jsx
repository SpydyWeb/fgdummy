...
const ULIPCallType = (props) => {
  ...
  const { selectedSubType, customerData, POSContactData, details, requestModeLU,clientEnquiryData } = props;
  ...
  const [isProcessedorNot,setIsProcessedorNot] = useState(false);
  const [isLoader,setIsLoader] = useState(false);
  const [isToggleSwitch, setIsToggleSwitch] = useState(false)
const [isShowfunddate, setisShowfunddate] = useState(false);
...
const fromFundObj = {
  ...
  useEffect(() => {
    ...
    if (details?.policyDetailsObj?.planAndStatus?.productType !== "UL") {
      setAlertTitle("FundSwitch, Fund Value, Premium Redirection Not Allowed For NON Ulip Policy's");
    ...
    // else if (details?.policyDetailsObj?.planAndStatus?.productUIN === "133L081V03") {
    //   setAlertTitle("FundSwitch, Fund Value, Premium Redirection Not Allowed For NON Ulip Policy's");
    //   setNavigateTo("/advancesearch");
    //   setShowAlert(true);
    // }
    else {
     // getUnProceesedUnits();   //Thisa api call imtegrated by Naga Raju K - 08-04-2024
    }
  ...
  useEffect(() => {
    if (props?.EmailResponse?.IsEmailmanagent) {
      if (selectedSubType === 'switch'||selectedSubType === 'premiumredirection') {
        Data[selectedSubType]?.Initiate_RequestBY?.forEach(element => {
      ...
      Data[selectedSubType]?.Initiate_RequestBY1?.forEach(element => {
        if (element?.label === "Request Mode") {
          form.setFieldsValue({
            requestchannel: 4
          });
      ...
      if (!Data[selectedSubType]) {
        ...
        if (!Array.isArray(Data[selectedSubType]?.Additionalnoteforcustomer)) {
          Data[selectedSubType].Additionalnoteforcustomer = [];
      }
      Data[selectedSubType].Additionalnoteforcustomer = Data[selectedSubType].Additionalnoteforcustomer.filter(
          comment => comment.name !== "AdditionalNoteForCustomer"
      );
      Data[selectedSubType].Additionalnoteforcustomer.push(
  ...
  useEffect(() => {
    ...
    setDisableRequestForm(false);
    form.setFieldsValue({ datechoice: "current" }); 
  ...
  useEffect(() => {
   ...
   // const boeDetails = Data[selectedSubType]?.BOE_Details;
    ...
    //         });
    //     }
    if (validateOTPSuccess) {setIsDisableNewMobileNo(true);}
    // } else {
    //     if (boeDetails) {
    //         boeDetails.forEach(element => {
    //             if (element?.label === "New Mobile" || element?.label === "New Email" || element?.label === "New Work Number" || element?.label === "New Alternate Number") {
  ...
  const handleSwitchChange = (checked) => {
    //setIsChecked(checked);         //commented by Naga Raju 16-04-24
    setIsToggleSwitch(checked);
    getUnProceesedUnits(checked);
  };
  ...
  const getFundValue = (AllFundNames = []) => {
    setIsLoading(true);
      ...
      .then((val) => {
        if (val?.data?.responseBody?.errorcode === "0") {
          ...
          const res = val?.data?.responseBody?.fundValue?.map((ele) => {
            // Find the corresponding fund data
            const fundData = AllFundNames?.find(x => x.fund_Code === ele.fndshrtdsc);
          
            // Add the new properties to each object
            return {
              ...ele,  // Spread the existing properties
              ToPercentage_New: '',
              isChecked: false,
              disabled: true,
              fundFullName: fundData?.fund_Name ?? "",
              allow_Switch_Out: fundData?.allow_Switch_Out ?? false,
              allow_Switch_In: fundData?.allow_Switch_In ?? false, // Corrected this line
            };
          });
  ...
  const getUnProceesedUnits = (isChecked) => {
    setIsLoading(true);
    setShowAlert(false);
    let response = apiCalls.GETUNPROCEESEDUNITS(details?.policyDetailsObj?.identifiers?.policyNo);
    response
      .then((val) => {
        if (val?.data) {
          //const res = val?.data?.responseBody?.unprocessed;
          setIsProcessedorNot(val?.data?.responseBody?.unprocessed);
          if(val?.data?.responseBody?.unprocessed){
            setAlertTitle("");
            setAlertData(val?.data?.responseBody?.errormessage);
           // setNavigateTo("/advancesearch");
            setShowAlert(true);
          }
          // else if(val?.data?.responseBody?.isAIR){
          //   setAlertTitle("");
          //   setAlertData((selectedSubType === "switch" || selectedSubType === "fundvalue") ? "Fund Switch not permitted on this policy" : "Premium Redirection not permitted on this policy");
          //   setShowAlert(true);
          // }
          else {
  ...
  const handleRadioChange = (e, item) => {
    const emailDetails=selectedSubType?.Request_Details;
    ...
    if (
      ...
      e.target.value === "backdated"
    ) {
      setisShowfunddate(true);
    } else if (
      item?.label?.includes("Date") &&
      e.target.value === "current"
    ) {
      setisShowfunddate(false);
    }
...
const convertDate1 = (inputDate) => {
  ...
  const GetCurrentFundAllocationn = async (ToFundVal = []) => {
    try {
      const response = await apiCalls.GetCurrentFundAllocation(details?.policyDetailsObj?.identifiers?.policyNo ||
      ...
      if (response?.data) {
        if (response?.data?.responseBody?.errorcode === '0') {
          ...
          let fundsWithName = units?.map(x => {
              const matchedFund = ToFundVal?.find(y => y.fund_Code === x.uniT_ALLOC_FUND) || {};
              
              return {
                  ...x,
                  uniT_ALLOC_FUND: matchedFund.fund_Name ?? x.uniT_ALLOC_FUND,
                  allow_Switch_Out: matchedFund.allow_Switch_Out ?? false,
                  allow_Switch_In: matchedFund.allow_Switch_In ?? false
              };
          });
  ...
  const handleSubmit = (values) => {
    ...
    setShowAlert(false)
    if (selectedSubType === "switch" || selectedSubType === "fundvalue") {
...
if(isShowPOSScreen){
  ...
  const getPlanFundDetails = () => {
    ...
    let obj = {
      Plan_Code: details?.policyDetailsObj?.planAndStatus?.planCode
    };
...
{
  "Status": "Create",
  "TagName": "EffectiveDate",
  "TagValue": values?.FundDate
    ? dayjs(values.FundDate).format("YYYYMMDD")
    ...
    // } 
    if (selectedSubType === "switch" || selectedSubType === "fundvalue") {
      const existingTags = new Set(obj.TransactionData.map((item) => item.TagName));
      fundValueData?.forEach((ele, index) => {
        index++;
    
        const fundFromTag = `FundFrom_${index}`;
        const percentFromTag = `PercentFrom_${index}`;
        const fundToTag = `FundTo_${index}`;
        const percentToTag = `PercentTo_${index}`;
    
        if (!existingTags.has(fundFromTag)) {
          obj.TransactionData.push({
            Status: "Create",
            TagName: fundFromTag,
            TagValue: ele.vrtfund
          });
          existingTags.add(fundFromTag);
        }
    
        if (!existingTags.has(percentFromTag)) {
          obj.TransactionData.push({
            Status: "Create",
            TagName: percentFromTag,
            TagValue: ele.ToPercentage_New ? ele.ToPercentage_New : 0
          });
          existingTags.add(percentFromTag);
        }
    
        if (!existingTags.has(fundToTag)) {
          obj.TransactionData.push({
            Status: "Create",
            TagName: fundToTag,
            TagValue: ele.vrtfund
          });
          existingTags.add(fundToTag);
        }
    
        if (!existingTags.has(percentToTag)) {
          obj.TransactionData.push({
            Status: "Create",
            TagName: percentToTag,
            TagValue: 0
          });
          existingTags.add(percentToTag);
        }
      });
      ...
      ToFundVal?.forEach((ele, index) => {
        if (ele.isChecked && ele.ToPercentage_New) {
          let existingFundToIndex = null;
      
          // Find existing FundTo_* index
          for (let item of obj.TransactionData) {
            const match = item.TagName.match(/^FundTo_(\d+)$/);
            if (match && item.TagValue === ele.fund_Code) {
              existingFundToIndex = match[1];
              break;
            }
          }
          ...
          if (existingFundToIndex !== null) {
            updatedIndex = existingFundToIndex;
          } else {
            indd++;
            updatedIndex = fundValueData.length + indd;
      
            const fundFromTag = `FundFrom_${updatedIndex}`;
            const percentFromTag = `PercentFrom_${updatedIndex}`;
          ...
          const fundToTag = `FundTo_${updatedIndex}`;
          const percentToTag = `PercentTo_${updatedIndex}`;
      
          // Add FundTo if it doesn't exist yet
          if (!existingTags.has(fundToTag) && existingFundToIndex === null) {
            obj.TransactionData.push({
              Status: "Create",
              TagName: fundToTag,
              TagValue: ele.fund_Code
          ...
          // ADD or UPDATE PercentTo
          const existingPercentTo = obj.TransactionData.find(item => item.TagName === percentToTag);
          if (existingPercentTo) {
            existingPercentTo.TagValue = ele.ToPercentage_New; // ✅ update
          } else {
            obj.TransactionData.push({
              Status: "Create",
              TagName: percentToTag,
              TagValue: ele.ToPercentage_New // ✅ insert
            });
            existingTags.add(percentToTag);
          }
        }
      ...
      PremiumRedirection.forEach((x,i) => {
        obj.TransactionData.push(
          {
            "Status": "Create",
            "TagName": `CurrentFund_${i+1}`,
            "TagValue": x.uniT_ALLOC_FUND
          },
          {
            "Status": "Create",
            "TagName": `CurrentPercentage_${i+1}`,
            "TagValue": x.uniT_ALLOC_PERC_AMT
          }
        )
      })
  ...
  return (
    <>
      ...
      >
        ...
        {(selectedSubType === "switch" || selectedSubType === "fundvalue") && (
          <>
            {!isShowPOSScreen && (
              <>
                {renderDetailsForm(
                  ...
                  <tbody>
                    {!isShowPOSScreen && (
                      <>
                        ...
                        <tr>
                      <td colSpan="5" style={{ textAlign: "left", fontWeight: "bold" }}>Total Fund Value</td>
                      <td style={{ fontWeight: "bold" }}>
                        {fundValueData.reduce((total, row) => total + parseFloat(row.curuntval), 0).toFixed(2)}
                      </td>
                      </tr> 
                      </>
                    )}
            ...
            {isChecked && (
              <>
                      ...
                      <div className="table-container email-table">
                        ...
                        <table className="responsive-table">
                          ...
                          <tbody>
                            {!isShowPOSScreen && (
                              <>
                                {fundValueData?.filter(row => parseFloat(row.curuntbal) !== 0)
                                .map((row, index) => (
                                  <tr key={row.id} className="nominee-input">
                                    <td>
                                      <Form.Item
                                        ...
                                        className="inputs-label mb-0"
                                      >
                                        <Checkbox checked={row.isChecked} onChange={(e) => handleCheckboxChange(e, index)} disabled={!row?.allow_Switch_Out} />
                                      </Form.Item>
                                    </td>
                                    <td>{row.fundFullName}</td>
                                    <td>
                                      ...
                                      >
                                        <Input
                                          ...
                                          disabled={row.disabled || validateOTPSuccess}
                              ...
                              <tr key={row.id} className="nominee-input">
                                <td>
                                  ...
                                  >
                                    ...
                                    <Checkbox checked={row.isChecked} onChange={(e) => handleCheckboxChange2(e, index)} disabled={!row?.allow_Switch_In}/>
                                  </Form.Item>
                                </td>
                                <td>{row.fund_Name}</td>
                                <td>
                ...
                {isShowRequestForms && (
                 ...
                 {isShowfunddate && (
                  <>{renderDetailsForm("Effective_Date")}</>
                )}
              </>
            )}
          </>
                                ...
                                <tr key={row.id} className="nominee-input">
                                  <td>
                                    ...
                                    >
                                      ...
                                      <Checkbox checked={row.isChecked} onChange={(e) => handleCheckboxChange3(e, index)} disabled={!row?.allow_Switch_In}/>
                                    </Form.Item>
                                  </td>
                                  <td>{row.fund_Name}</td>
                                  <td>
        ...
        {selectedSubType === "fundvalue" && renderDetailsForm("Initiate_RequestBY1")}
        ...
        <div className="contact-details-btn">
          ...
          {/* {selectedSubType !=="fundvalue" && ( */}
          {selectedSubType !=="fundvalue" && isToggleSwitch && (
            <Button
              type="primary"
